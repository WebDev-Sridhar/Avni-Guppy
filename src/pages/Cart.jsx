...
const hasStockError = cartItems.some(item => item.latestStock !== undefined && item.qty > item.latestStock);
  ...
  return (
    <div className="p-6 max-w-full min-h-dvh m-auto bg-white-50  rounded-lg shadow-md  lg:max-w-2xl">
      ...
      {cart.length === 0 ? (
        ...
        <>
         ...
         <div className="flex items-center mt-2 space-x-2 mb-2">
          ...
          <p className="font-md text-gray-600">Qty:</p>
                <button
...
<div>
 ...
 {item.latestStock > 0 && item.qty > item.latestStock && (
  ...
  <p className="text-green-600 text-md mt-3">You save ₹{discount} on this order</p>
            ...
            <a
              onClick={clearCart}
              className="text-red-500  py-2 rounded hover:text-red-600 hover:underline cursor-pointer text-right"
            >
              Clear Cart
            </a>
          </div>
      ...
      {cart.length > 0 && (
        <div className="mt-6">
           { hasStockError ? 
           <>
          <a className="bg-[#bdc2c0] text-white px-6 py-3 rounded-md font-semibold ">
            <button className="text-white-500 text-sm hover:underline disabled:">
              Proceed to Checkout
            </button>
          </a>
          <p className="text-sm py-4 font-normal text-red-700">Some products are out of stock, adjust quantity or remove the item</p>
           ...
           :
            <Link
            to="/cart/checkout"
            className="bg-[#486759] text-white px-6 py-3 rounded-md font-semibold hover:bg-[#375b4b]"
          >
