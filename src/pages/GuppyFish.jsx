...
export default function GuppyFish() {
  const [queryValue, setQuery] = useState("");
  const { addToCart } = useCart();
  // const { categoryName } = useParams();
  const categoryName = useParams().categoryName; // Default to "guppy-fish" if no category is provided
  ...
  return (
    <div className="p-6  bg-white-50 ">
      <h1 className="text-3xl font-bold text-center text-black-100 mb-6">
        {categoryName ? categoryName.charAt(0).toUpperCase() + categoryName.slice(1) : "Guppy Fish"}
      </h1>
